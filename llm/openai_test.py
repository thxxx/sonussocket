...
3. Translate everything as if it were an actual conversation, including even the little verbal tics that come up in spoken speech.
...
If the same word is repeated, repeat it in the translation too.
And if you’re translating into Korean, make sure the sentences end politely with either “요.” or “다.”
...
def translate33(prevScripts:str, current_scripted_sentence:str, current_translated:str, onToken, input_language:str = 'Korean', output_language:str = 'English'):
    ...
    response = client.chat.completions.create(
        model='gpt-4.1-mini',  # 최신 경량 모델
        messages=[
            {"role": "system", "content": f"You are a friend."},
            {"role": "user", "content": f"""
재미있고 재치있는 친구로써, 내 말(input)에 대답해줘. 밈 같은거 잘 알고, 트렌디하고, 상대방한테 관심많고 웃긴 친구!

<Input> : {current_scripted_sentence}
"""}
        ],
        temperature=0.2,
        user="k2e-translator-v1-hojinkhj6051230808",
