        ...
        async def consume_loop():
            while sess.running:
                ...
                if isinstance(text_chunk, tuple) and len(text_chunk) == 2 and text_chunk[0] == "__silence__":
                    try:
                        silence_sec = float(text_chunk[1])*0.6
                        if silence_sec > 0:
                            num_samples = int(silence_sec * sr)
                            silence_wav = torch.zeros(1, num_samples, dtype=torch.float32)  # mono, 24kHz
                            # 무음을 '먼저' 내보냄
                ...
                if matched is not None:
                    ...
                    try:
                        ...
                        print(f"[starter] played '{sample_path}'")
                    except Exception as e:
                ...
                while True:
                    ...
                    if evt_type == "chunk":
                        ...
                        schedule_silence_nudge(sess, delay=8.0)
...
def schedule_silence_nudge(sess: Session, delay: float = 5.0):
